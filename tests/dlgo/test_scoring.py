...
from dlgo.scoring import Territory, GameResult
...
def test_large_territory():
    territory_map = {Point(i, j): status for i in range(1, 10) for j in range(1, 10)
                     for status in [Player.black, Player.white, 'territory_b', 'territory_w', 'dame']}
    territory = Territory(territory_map)
    assert territory.num_black_territory + territory.num_white_territory + \
           territory.num_black_stones + territory.num_white_stones + territory.num_dame == 81
...
@pytest.mark.parametrize("b, w, komi, expected_winner, expected_margin, expected_str", [
