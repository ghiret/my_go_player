# Copied from https://github.com/maxpumperla/deep_learning_and_the_game_of_go/blob/master/code/dlgo/gosgf/sgf.py
"""
...
class Node:
    ...
    def _set_raw_list(self, identifier, values):
        if identifier == b"SZ" and values != [str(self._presenter.size).encode(self._presenter.encoding)]:
    ...
    def __str__(self):
        ...
        def format_property(ident, values):
            return ident.decode(encoding) + "".join("[%s]" % s.decode(encoding) for s in values)
...
class Sgf_game:
    ...
    def get_player_name(self, colour):
        ...
        """
        try:
            return self.root.get({"b": b"PB", "w": b"PW"}[colour]).decode(self.presenter.encoding)
        except KeyError:
            return None
